...
3. Commit and push the change onto master branch, and wait until CI finishes successfully.
4. Tag the head of master with the version number in the format `vx.x.x` - e.g.: `git tag v0.20.4`.
5. Push the tag to remote: `git push origin vX.Y.Z` - this will start CI process that will create a GitHub release and publish the packages to NuGet.
