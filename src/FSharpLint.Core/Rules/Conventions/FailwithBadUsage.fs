    ...
    let rec checkExpr node maybeIdentifier =
        ...
        | SynExpr.App (_, _, SynExpr.Ident failwithId, expression, range) when
            ...
            | SynExpr.Const (SynConst.String (id, _, _), _) when id = String.Empty ->
                generateViolation failwithId.idText id range BadUsageType.EmptyMessage maybeIdentifier
            | SynExpr.Const (SynConst.String (id, _, _), _) when id <> fakeExternDeclErrorMsg ->
                ...
                if isDuplicate then
                    generateViolation failwithId.idText id range BadUsageType.DuplicateMessage maybeIdentifier
                else
                    match maybeIdentifier with
                    | Some maybeId ->
                        generateViolation failwithId.idText id range BadUsageType.SwallowedException (Some maybeId)
                    | _ ->
                        failwithMessages <-
            ...
            | SynExpr.LongIdent (_, SynLongIdent (id, _, _), _, _) when
                ...
                generateViolation
                    ...
                    (None)
            | SynExpr.Null range ->
                generateViolation failwithId.idText "null" range BadUsageType.NullMessage maybeIdentifier
            | _ -> Array.empty
