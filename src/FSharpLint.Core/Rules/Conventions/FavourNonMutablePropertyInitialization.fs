    ...
    let rec traverse (declarations: List<SynModuleDecl>) (classInstances: List<string>) (instancesWithMethodCall: List<string>) =
        ...
        | SynModuleDecl.DoExpr(_, expression, _)::rest ->
            ...
            | SynExpr.LongIdentSet(LongIdentWithDots(identifiers, _), _, _) ->
                if not identifiers.IsEmpty && containsInstance identifiers.[0].idText classInstances then
                    if not (containsInstance identifiers.[0].idText instancesWithMethodCall) then
                        match List.tryLast identifiers with
                        | Some ident -> getWarningDetails ident
                        | None -> failwith "list was already checked to be sure that it is not empty, report this bug!"
                    else
                        Array.empty
                else
                    Array.empty
        ...
        | SynModuleDecl.Let(_, bindings, _)::rest ->
            ...
            | SynBinding(_, _, _, _, _, _, _, _, _, SynExpr.LetOrUse(_, _, bindings, SynExpr.Sequential(_, _, app, SynExpr.LongIdentSet(LongIdentWithDots(identifiers, _), _, _), _), _), _, _)::nrest ->
                ...
                if not identifiers.IsEmpty && containsInstance identifiers.[0].idText instances then
