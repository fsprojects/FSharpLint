...
module HintMatcher =
    ...
    type HintsRule(analyserName, name, code, config:Configuration.Configuration) =
        inherit Rule(analyserName, name, code, config)
        
        let hints =
            let analyser = Map.find analyserName config.Analysers

            match Map.tryFind name analyser.Settings with
    ...
    module Analysis =
        ...
        let hintError hint analysisArgs range =
            ...
            let error =
                ...
                | Suggestion.Expr(expr) -> 
                    ...
                    let errorFormatString = Resources.GetString("HintRefactor")
                    System.String.Format(errorFormatString, matched, suggestion)
                | Suggestion.Message(message) -> 
                    let errorFormatString = Resources.GetString("HintSuggestion")
        ...
        let confirmFuzzyMatch analysisArgs (node:AbstractSyntaxArray.Node) breadcrumbs (hint:HintParser.Hint) =
            ...
            | AstNode.Pattern(pattern), HintPat(hintPattern) ->
                if MatchPattern.matchHintPattern (pattern, hintPattern) then
                    hintError hint analysisArgs pattern.Range
            | AstNode.Expression(expr), HintExpr(hintExpr) -> 
                let arguments =
                    { MatchExpression.LambdaArguments = Map.ofList []
                      ...
                      MatchExpression.FSharpCheckFileResults = analysisArgs.CheckResults
                ...
                if MatchExpression.matchHintExpr arguments then
                    ...
                    | HintExpr(Expression.Lambda(_)), Suggestion.Expr(Expression.Identifier(_)) -> 
                        if lambdaCanBeReplacedWithFunction analysisArgs.CheckResults breadcrumbs expr.Range then
                            hintError hint analysisArgs expr.Range
                    | _ ->
                        hintError hint analysisArgs expr.Range
            | _ -> ()

        let analyse (rule:HintsRule) analysisArgs = 
            let maxBreadcrumbs = 6

            let confirmFuzzyMatch i =
                let breadcrumbs = getBreadcrumbs maxBreadcrumbs analysisArgs i
                if rule.NotSuppressed analysisArgs i then
                    let node = analysisArgs.SyntaxArray.[i]
                    confirmFuzzyMatch analysisArgs node breadcrumbs
                else
                    ignore

            FuzzyHintMatcher.possibleMatches analysisArgs rule.Hints confirmFuzzyMatch
    ...
    type HintsAnalyser(config) =
        ...
        override this.Analyse analysisArgs = 
            Analysis.analyse this.Hints analysisArgs