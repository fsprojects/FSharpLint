...
module AbstractSyntaxArray =
    ...
    let isPublic (syntaxArray:Node []) (skipArray:Skip []) i =
        ...
        let rec isPublic publicSoFar isBinding i =
            if i = 0 then publicSoFar
                ...
                | Pattern(SynPat.LongIdent(_, _, _, _, access, _)) ->
                    isPublic (isSynAccessPublic access) isBinding skipArray.[i].ParentIndex
                | TypeSimpleRepresentation(_)
                | Pattern(_) -> true
                | Binding(SynBinding.Binding(access, _, _, _, _, _, _, _, _, _, _, _)) ->
                    isPublic (isSynAccessPublic access) true skipArray.[i].ParentIndex
                | MemberDefinition(_) -> isPublic publicSoFar isBinding skipArray.[i].ParentIndex
                ...
                | Identifier(_)
                | SimplePattern(_)
                | SimplePatterns(_) -> isPublic publicSoFar isBinding skipArray.[i].ParentIndex
                | TypeDefinition(_)
                | Expression(_) -> not isBinding && isPublic publicSoFar isBinding skipArray.[i].ParentIndex
            else false
