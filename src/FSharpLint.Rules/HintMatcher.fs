...
module HintMatcher =
    ...
    module MatchExpression =
        ...
        and private matchLambda arguments =
            match (arguments.Expression, arguments.Hint) with
            | SynExpr.Lambda(_) as lambda, Expression.Lambda(lambdaArgs, LambdaBody(body)) -> 
                ...
                | LambdaMatch.Match(bodyExpr, lambdaArguments, numberOfWildcards) -> 
                    ...
                    | Some(bodyExpr) -> 
                        { arguments.SubHint(bodyExpr, body) with LambdaArguments = lambdaArguments } |> matchHintExpr
                    | None -> false
