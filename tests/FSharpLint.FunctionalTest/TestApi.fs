...
module TestApi =
    ...
    type TestApi() =
        ...
        member __.``Lint solution via absolute path``(solutionFileName: string, expectedWarnings: int) =
        ...
        member __.``Lint solution via relative path``(solutionFileName: string, expectedWarnings: int) =
            let projectPath = basePath </> "tests" </> "FSharpLint.FunctionalTest.TestedProject"
            let solutionFile = projectPath </> solutionFileName
