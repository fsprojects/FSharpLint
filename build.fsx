...
let exec cmd args dir =
    let proc =
        ...
        |> CreateProcess.ensureExitCodeWithMessage $"Error while running '{cmd}' with args: {args}"
...
let PackageReleaseNotes baseProps =
    if isTag then
        ("PackageReleaseNotes", $"{gitUrl}/blob/v{nugetVersion}/CHANGELOG.md")::baseProps
    else
        baseProps
    ...
    | Some "push" ->
        ...
        | Some key ->
            if isTag then
                ...
                | Some commitHash ->
                    let gitArgs = $"describe --exact-match --tags {commitHash}"
                    let proc =
...
Target.create "SelfCheck" (fun _ ->
    ...
    exec "dotnet" $"run lint {sol.FullName}" consoleProj.Directory.FullName
)
